...
function fn() {
  ...
  $.euroclip.DEBUG = false;
  $.euroclip.HOLDER = $('#detail .c1303').first();
  $.euroclip.WEB_PREFIX = "";

  if (window.location.hostname.indexOf(".sk") > 0) {
    $.euroclip.LANG = "sk";
    $.euroclip.SCRIPT_SOURCE = 'https://www.dantik.sk/fotky9862/fotok/data/';
    $.euroclip.SHOP_SOURCE = 'https://www.dantik.sk/fotky9862/fotok/';
    ...
    $.euroclip.LANG = "cz";
    $.euroclip.SCRIPT_SOURCE = 'https://www.dantik.cz/fotky21110/fotok/data/';
    $.euroclip.SHOP_SOURCE = 'https://www.dantik.cz/fotky21110/fotok/';
  ...
  if (!sessionStorage.getItem('config')) {
    var getConfig = $.ajax({
      crossDomain: true,
      type: 'GET',
      url: $.euroclip.SHOP_SOURCE + 'config.csv',
      dataType: "json",
      success: function (data) {
  ...
  if (href == "/" + $.euroclip.ADDR_BASKET) {
    ...
    $.ajax({
      crossDomain: true,
      type: 'GET',
      url: $.euroclip.SHOP_SOURCE + 'config_cart.csv',
      dataType: "json",
      success: function (data) {
...
//                console.log("Glass - křehké!");
    ...
    var template = $.ajax({
      crossDomain: true,
      type: 'GET',
      url: $.euroclip.SCRIPT_SOURCE + 'tensioning_template.csv',
      dataType: "html",
      success: function (data) {
    ...
    var specific_model = $.ajax({
      crossDomain: true,
      type: 'GET',
      url: $.euroclip.SCRIPT_SOURCE + 'tensioning_model.csv',
      dataType: "script"
    });
    ...
    $.euroclip.setup = function () {
      ...
      for (var i = 0; i < $.euroclip.products.length; i++) {
        if ("/" + $.euroclip.WEB_PREFIX + $.euroclip.products[i].url === href) {
